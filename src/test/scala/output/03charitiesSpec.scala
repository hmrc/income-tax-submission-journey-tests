...
class charitiesSpec extends BaseSpec {
  ...
  def individualUser(returning: Boolean = false) = Seq(Map(
    "Redirect url" -> "/InYear/start",
    "Credential Strength" -> "strong",
    "Confidence Level" -> "250",
    "Affinity Group" -> "Individual",
    "Nino" -> (if (returning) "AA123459A" else "AA123456A"),
    "Enrolment Key 1" -> "HMRC-MTD-IT",
    "Identifier Name 1" -> "MTDITID",
    "Identifier Value 1" -> (if (returning) "1234567891" else "1234567890")
  ))

  def agentUser(returning: Boolean = false) = Seq(Map(
    "Redirect url" -> s"/test-only/InYear/additional-parameters?ClientNino=${if (returning) "AA123459A" else "AA123457A"}&ClientMTDID=1234567890",
  ...
  val agentUserWithPriorData = Seq(Map(
